(async () => {
  'use strict';
  const VERSION = 'ATHOS_V41_GAME_FEEL_TEST_001';
  ...
  function panel(){ let p=document.getElementById('athos-v41-test-panel'); if(!p){p=document.createElement('div'); p.id='athos-v41-test-panel'; p.style.cssText='position:fixed;right:10px;top:10px;width:min(540px,calc(100vw - 20px));max-height:84vh;overflow:auto;z-index:99999999;background:rgba(0,0,0,.94);color:#fff;font:13px Arial;border:3px solid #55ff55;border-radius:10px;padding:10px;box-shadow:0 10px 35px rgba(0,0,0,.55)'; document.body.appendChild(p);} const ok=results.filter(r=>r.status.includes('OK')).length, fail=results.filter(r=>r.status.includes('FALHOU')).length, av=results.filter(r=>r.status.includes('AVISO')).length; p.innerHTML=`<b>${VERSION}</b><div>✅ ${ok} ❌ ${fail} ⚠️ ${av} JS:${errors.length}</div>`+results.slice(-55).map(r=>`<div style="border-top:1px solid #444;padding:4px"><b>${r.status}</b> ${r.teste}<br><small>${String(r.detalhe).replace(/[<>&]/g,'')}</small></div>`).join(''); }
  async function fetchText(path){ try{ const r=await fetch(path+'?v41test='+Date.now(),{cache:'no-store'}); return r.ok ? await r.text() : ''; }catch{return '';} }
  async function checkFile(path){ try{ const r=await fetch(path+'?v41test='+Date.now(),{method:'HEAD',cache:'no-store'}); add(`Arquivo ${path}`, r.ok, `HTTP ${r.status}`); }catch(e){ add(`Arquivo ${path}`, false, e.message); } }
  ...
  console.log('%cATHOS V41 GAME FEEL TESTE', 'font-size:20px;color:#55ff55;background:#111;padding:8px');
  ...
  add('Título V41 Game Feel', /V41|GAME FEEL|Game Feel/i.test(document.title + index), document.title);
  add('app.js grande carregado', app.length > 50000, `${app.length} caracteres`);
  add('Camada V41 no app.js', /GAME_FEEL|joystickDeadzone|platformSnap|jumpBufferMs|stopHorizontalIfNoDirectionalInput/.test(app), 'buscando game feel V41');
  ...
  console.log('%cRELATÓRIO FINAL V41', 'font-size:18px;color:#ffcc00;background:#111;padding:8px'); console.table(results); window.ATHOS_V41_TEST_RESULTS=results; window.ATHOS_V41_TEST_ERRORS=errors;
  alert(`Teste V41 finalizado.\nOK: ${results.filter(r=>r.status.includes('OK')).length}\nFalhas: ${results.filter(r=>r.status.includes('FALHOU')).length}\nAvisos: ${results.filter(r=>r.status.includes('AVISO')).length}\nErros JS: ${errors.length}`);
})();
