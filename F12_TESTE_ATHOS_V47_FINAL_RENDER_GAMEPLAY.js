...
console.log('%cATHOS V47.1 RENDER FIX + GAMEPLAY','font-size:20px;color:#55ff55;background:#111;padding:8px');
...
add('Título V47.1',/V47\.1|RENDER FIX|Render \+ Gameplay/i.test(document.title+index),document.title);
...
add('V47.1 instalado',st&&st.installed===true,JSON.stringify(st||{}));
add('V47.1 criou render rico',st&&st.hasGroup===true&&Number(st.objects||0)>40,JSON.stringify(st||{}));
...
console.table(results); window.ATHOS_V471_TEST_RESULTS=results; window.ATHOS_V471_TEST_ERRORS=errors;
alert(`Teste V47.1 finalizado.\nOK: ${results.filter(r=>r.status.includes('OK')).length}\nFalhas: ${results.filter(r=>r.status.includes('FALHOU')).length}\nErros JS: ${errors.length}`);
})();
