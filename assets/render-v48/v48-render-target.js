...
(function(){
  'use strict';
  const VERSION = 'V48_RENDER_ALVO_FIEL_SHADER_SAFE_10_10';
  ...
  const api={version:VERSION, install:safe(function(ctx){if(installed)return; if(!ctx||!ctx.THREE||!ctx.scene)return; installed=true; document.body.classList.add('v48-target-active');}), update:safe(function(ctx,dt){if(!installed||!group)return;dt=dt||.016;updatables.forEach(fn=>fn(dt));}), rebuildWorld:safe(function(ctx,worldName){if(!installed||!ctx||!ctx.THREE||!ctx.scene)return; currentWorld=norm(worldName); this.disposeVisualsOnly(); if(currentWorld==='real'){restoreOriginals();return} const THREE=ctx.THREE,pal=PAL[currentWorld]||PAL.campo; init(THREE,pal); group=new THREE.Group(); group.name='V48_RENDER_TARGET_GROUP'; ctx.scene.add(group); resetStats(); hideOriginals(ctx); setupScene(THREE,ctx,pal,currentWorld); const len=Math.min(240,Math.max(150,Number(ctx.level&&ctx.level.length||220))); makeTerrain(THREE,pal,currentWorld,len); for(let z=-8;z>-len+20;z-=24){crystal(THREE,(Math.random()>.5?1:-1)*(1.2+Math.random()*2.1),1.15,z,false)} enemies(THREE,len); waterLava(THREE,pal,currentWorld); floating(THREE,pal); portal(THREE,pal,-len+20);}), disposeVisualsOnly:safe(function(){if(group){group.traverse(o=>{if(o.geometry&&o.geometry.dispose)o.geometry.dispose();}); if(group.parent)group.parent.remove(group)} group=null; updatables=[];}), dispose:safe(function(){this.disposeVisualsOnly(); restoreOriginals(); installed=false; document.body.classList.remove('v48-target-active');}), getStatus:function(){return{version:VERSION,installed,world:currentWorld,hasGroup:!!group,objects:stats.objects,animated:stats.animated,hiddenOriginals:stats.hiddenOriginals,shaderSafe:true,textureSamplers:0,target:'1000801721_reference_style'}}};
