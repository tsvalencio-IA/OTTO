(async () => {
  'use strict';
  const VERSION = 'ATHOS_V411_POINTER_HOTFIX_TEST_001';
  ...
  function panel(){ let p=document.getElementById('athos-v411-test-panel'); if(!p){p=document.createElement('div'); p.id='athos-v411-test-panel'; p.style.cssText='position:fixed;right:10px;top:10px;width:min(540px,calc(100vw - 20px));max-height:84vh;overflow:auto;z-index:99999999;background:rgba(0,0,0,.94);color:#fff;font:13px Arial;border:3px solid #55ff55;border-radius:10px;padding:10px;box-shadow:0 10px 35px rgba(0,0,0,.55)'; document.body.appendChild(p);} const ok=results.filter(r=>r.status.includes('OK')).length, fail=results.filter(r=>r.status.includes('FALHOU')).length, av=results.filter(r=>r.status.includes('AVISO')).length; p.innerHTML=`<b>${VERSION}</b><div>✅ ${ok} ❌ ${fail} ⚠️ ${av} JS:${errors.length}</div>`+results.slice(-55).map(r=>`<div style="border-top:1px solid #444;padding:4px"><b>${r.status}</b> ${r.teste}<br><small>${String(r.detalhe).replace(/[<>&]/g,'')}</small></div>`).join(''); }
  async function fetchText(path){ try{ const r=await fetch(path+'?v411test='+Date.now(),{cache:'no-store'}); return r.ok ? await r.text() : ''; }catch{return '';} }
  async function checkFile(path){ try{ const r=await fetch(path+'?v411test='+Date.now(),{method:'HEAD',cache:'no-store'}); add(`Arquivo ${path}`, r.ok, `HTTP ${r.status}`); }catch(e){ add(`Arquivo ${path}`, false, e.message); } }
  function pointer(el,type,dx=0,dy=0){ const r=el.getBoundingClientRect(); const x=r.left+r.width/2+dx, y=r.top+r.height/2+dy; try{el.dispatchEvent(new PointerEvent(type,{bubbles:true,cancelable:true,pointerId:41,pointerType:'touch',clientX:x,clientY:y,isPrimary:true}));}catch{} }
  ...
  function playerState(){ return window.ATHOS_TEST_API && window.ATHOS_TEST_API.getPlayerState ? window.ATHOS_TEST_API.getPlayerState() : null; }
  function inputState(){ return window.ATHOS_TEST_API && window.ATHOS_TEST_API.getInputState ? window.ATHOS_TEST_API.getInputState() : null; }

  console.log('%cATHOS V41.1 POINTER HOTFIX TESTE', 'font-size:20px;color:#55ff55;background:#111;padding:8px');
  ...
  const app = await fetchText('./app.js'); const index = await fetchText('./index.html');
  add('Título V41.1 Game Feel Hotfix', /V41\.1|V41|GAME FEEL|Game Feel/i.test(document.title + index), document.title);
  add('app.js grande carregado', app.length > 50000, `${app.length} caracteres`);
  add('Camada V41/V41.1 no app.js', /GAME_FEEL|joystickDeadzone|platformSnap|jumpBufferMs|stopHorizontalIfNoDirectionalInput|safePointerCapture/.test(app), 'buscando game feel V41');
  ...
  add('API de game feel existe', !!(window.ATHOS_TEST_API && window.ATHOS_TEST_API.getGameFeel), 'ATHOS_TEST_API.getGameFeel');
  if(window.ATHOS_TEST_API && window.ATHOS_TEST_API.getGameFeel){ const gf=window.ATHOS_TEST_API.getGameFeel(); add('Game feel expõe parâmetros', !!(gf.joystickDeadzone && gf.platformSnap && gf.jumpBufferMs), JSON.stringify(gf)); }
  ...
  const quiz = document.getElementById('quizBtn') || findButton(/quiz/i); await tap(quiz,'Abrir Quiz'); await sleep(400); const m = modal() || document; const opts = all('button,.quiz-option,.choice,[data-correct]',m).filter(visible).filter(o=>!/fechar|close/i.test(txt(o))); add('Quiz abre no lobby', opts.length >= 2, `${opts.length} opções`); await closeModal();
  const ask = document.getElementById('askBtn') || findButton(/falar|perguntar/i); await tap(ask,'Falar com Athos'); await sleep(350); const m2=modal()||document; add('Falar abre no lobby', !!all('input,textarea',m2).find(visible), 'input'); await closeModal();
  ...
  console.log('%cRELATÓRIO FINAL V41.1', 'font-size:18px;color:#ffcc00;background:#111;padding:8px'); console.table(results); window.ATHOS_V411_TEST_RESULTS=results; window.ATHOS_V411_TEST_ERRORS=errors;
  alert(`Teste V41.1 finalizado.\nOK: ${results.filter(r=>r.status.includes('OK')).length}\nFalhas: ${results.filter(r=>r.status.includes('FALHOU')).length}\nAvisos: ${results.filter(r=>r.status.includes('AVISO')).length}\nErros JS: ${errors.length}`);
})();
