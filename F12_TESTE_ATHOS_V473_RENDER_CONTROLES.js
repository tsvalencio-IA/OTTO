...
async function head(p){try{const r=await fetch(p+'?v473test='+Date.now(),{method:'HEAD',cache:'no-store'});add(`Arquivo ${p}`,r.ok,`HTTP ${r.status}`)}catch(e){add(`Arquivo ${p}`,false,e.message)}}
...
console.log('%cATHOS V47.3 RENDER ALVO + CONTROLES SHADER SAFE','font-size:20px;color:#55ff55;background:#111;padding:8px');
...
add('Título V47.3',/V47\.3|RENDER ALVO/i.test(document.title),document.title);
add('Módulo V47 existe',!!window.ATHOS_V47_RENDER_PREMIUM,window.ATHOS_V47_RENDER_PREMIUM?.version||'ausente');
add('CSS alvo V47.3 carregado',/V47\.3/.test(await (await fetch('./assets/render-v47/v47-render-premium.css?v='+Date.now())).text()),'override de controles/render');
...
console.table(results); window.ATHOS_V473_TEST_RESULTS=results; window.ATHOS_V473_TEST_ERRORS=errors;
alert(`Teste V47.3 finalizado.\nOK: ${results.filter(r=>r.status.includes('OK')).length}\nFalhas: ${results.filter(r=>r.status.includes('FALHOU')).length}\nErros JS: ${errors.length}`);
})();