(async()=>{
'use strict';
const VERSION='ATHOS_V462_RENDER_PREMIUM_AR_HOTFIX_TESTE';
...
function panel(){let p=document.getElementById('athos-v462-test-panel');if(!p){p=document.createElement('div');p.id='athos-v462-test-panel';p.style.cssText='position:fixed;right:10px;top:10px;width:min(590px,calc(100vw - 20px));max-height:84vh;overflow:auto;z-index:99999999;background:rgba(0,0,0,.94);color:#fff;font:13px Arial;border:3px solid #55ff55;border-radius:10px;padding:10px';document.body.appendChild(p)}const ok=results.filter(r=>r.status.includes('OK')).length,fail=results.filter(r=>r.status.includes('FALHOU')).length;p.innerHTML=`<b>${VERSION}</b><div>✅ ${ok} ❌ ${fail} JS:${errors.length}</div>`+results.slice(-80).map(r=>`<div style="border-top:1px solid #444;padding:4px"><b>${r.status}</b> ${r.teste}<br><small>${r.detalhe.replace(/[<>&]/g,'')}</small></div>`).join('')}
async function fetchText(p){try{const r=await fetch(p+'?v462test='+Date.now(),{cache:'no-store'});return r.ok?await r.text():''}catch{return''}}
async function head(p){try{const r=await fetch(p+'?v462test='+Date.now(),{method:'HEAD',cache:'no-store'});add(`Arquivo ${p}`,r.ok,`HTTP ${r.status}`)}catch(e){add(`Arquivo ${p}`,false,e.message)}}
function pointer(el,type){const r=el.getBoundingClientRect();try{el.dispatchEvent(new PointerEvent(type,{bubbles:true,cancelable:true,pointerId:462,pointerType:'touch',clientX:r.left+r.width/2,clientY:r.top+r.height/2,isPrimary:true}))}catch{}}
...
console.log('%cATHOS V46.2 RENDER PREMIUM AR HOTFIX TESTE','font-size:20px;color:#55ff55;background:#111;padding:8px');
...
const app=await fetchText('./app.js'), index=await fetchText('./index.html'), v46=await fetchText('./assets/render-v46/v46-render-premium.js');
add('Título V46.2',/V46\.2|AR HOTFIX/i.test(document.title+index),document.title);
add('Módulo V46 carregado',!!window.ATHOS_V46_RENDER_PREMIUM,window.ATHOS_V46_RENDER_PREMIUM?.version||'ausente');
add('Safe wrapper preservado',/fn\.apply\(this, args\)/.test(v46),'this preservado');
add('Hotfix AR sem ativar em desktop existe',/canSafelyActivateNativeAR/.test(app)&&/markNativeARUnavailable/.test(app),'não chama activateAR quando canActivateAR !== true');
add('getARViewer prioriza nativeViewer',/return els\.nativeViewer \|\| els\.arAnchorViewer/.test(app),'evita arAnchor oculto como principal');
add('Câmera fake não volta',/sem fallback fake|Câmera falsa não será usada|realBg = false/.test(app),'sem AR falso');
...
await tap(real,'Real/AR protegido'); await sleep(1200);
const ar=api.getARSafety?.();
add('AR seguro sem câmera falsa',ar&&ar.fakeCamera===false&&ar.realBg===false,JSON.stringify(ar||{}));
...
add('Clique Real não gerou erro ARRenderer/null.add',errors.length===beforeErrors,!errors.slice(beforeErrors).join(' | ')?'sem erro novo':errors.slice(beforeErrors).join(' | '));
add('Sem erro JavaScript capturado',errors.length===0,errors.slice(-5).join(' | '));
console.table(results); window.ATHOS_V462_TEST_RESULTS=results; window.ATHOS_V462_TEST_ERRORS=errors; alert(`Teste V46.2 finalizado.\nOK: ${results.filter(r=>r.status.includes('OK')).length}\nFalhas: ${results.filter(r=>r.status.includes('FALHOU')).length}\nErros JS: ${errors.length}`);
})();
